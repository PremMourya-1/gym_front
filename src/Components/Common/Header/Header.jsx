...
function Header() {
  ...
  const formattedDate = `${weekdays[date.getDay()]} ${date.getDate()} ${months[date.getMonth()]
    } ${date.getFullYear()}`;
  ...
  return (
    <>
      <header
        className={` ${theme.isSideBarSmall ? "ms-20 small" : " ms-[260px] md:ms-14 "
          } pt-5 pb-3 px-5 z-20 md:ms-12 md:pt-3 `}
      >
        <div className="headerDetails flex md:justify-end md:flex-col md:gap-4 md:items-start justify-between items-center">
          <div className=" md:hidden">
            <p className="font-medium text-xl text-primary mb-0.5 capitalize flex items-center gap-1 ">
              <button
                ...
                className={`${theme.isSideBarSmall ? "rotate-180" : "rotate-0"
                  } toggleButton group-hover:block `}
              >
                <FaAnglesLeft size={16} />
              </button>   {user && !isAdmin ? user.gymName : user?.fullName} ||     {user && !isAdmin ? user.ownerName : user?.fullName}
            </p>
          ...
          <div className="last flex gap-4  items-center md:w-full justify-end">
            <div className=" flex  items-center gap-3">
              <form
                ...
                className={`${isSearching ? "w-[200px]  me-3" : "w-0 overflow-hidden"
                  } transition3`}
              >
